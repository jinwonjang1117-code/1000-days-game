...
- `src/games/escapeTheMinions/` — "Escape the Minions", a 2-player online co-op roguelike. Directory, asset folder (`public/assets/escape-the-minions/`), and URL/route id (`TEAM_GAME_ID`) all match the public name; only the internal scene-key/PeerJS-id namespace (`TEAM_GAME_NAMESPACE = 'teamGame'`) stayed as a technical codename on purpose, see `gameId.ts`. **Has its own [CLAUDE.md](src/games/escapeTheMinions/CLAUDE.md) — read it before touching anything under that directory.** Don't modify the platformer's files while working on this game unless a task explicitly says to touch the shared main menu.
