...
### Angel Room (brainstorm — direction agreed, exact numbers/roster still open, notes dropped ahead of any stage actually covering this)

- A room that occasionally appears, at a **very low chance** — offering genuinely strong item choices, an Isaac-style counterpart to Devil's Room (which trades a cost for power; Angel Room is just a clean, high-quality pick with no downside, the reward for luck rather than risk).
- **Trigger (agreed): independent rare per-level roll**, unrelated to Devil's Room or boss performance — not the same no-hit-clear condition, not mutually exclusive with it. Two separate systems, not one branching one. Exact chance still open (ballpark 3-5%, needs real playtesting).
- **Item pool (agreed): a smaller curated subset, not the full strong-item pool** — hand-picked strong items *plus* role items once the role system (roadmap stage 7) exists, so Angel Room stays relevant/interesting after roles land instead of only ever offering stat items. Not an exclusive Angel-only item tier (unlike Devil's Room) — it's a curated slice of things that already exist elsewhere, just guaranteed and cost-free here.
- Still undecided: the actual curated roster (which strong items make the cut, and how role items slot in once they exist), how many options it offers per visit (2-3, matching Devil's Room's format, is the natural default), and its room-presence mechanic (portal off some trigger vs. just an ultra-rare floor-gen room slot like a rarer Golden Room).
